...
CSV_FILE_PATH = "RePro.csv"

CSV_TEXT_COLUMN = 'review_text'
CSV_TITLE_COLUMN = 'review_title'
CSV_LABEL_COLUMN = 'overall_rating'
...
def load_data_from_csv(file_path):
    """
    Carrega, combina, limpa e traduz os dados do arquivo CSV RePro.
    """
    ...
    df.dropna(subset=[FINAL_TEXT_COLUMN, CSV_LABEL_COLUMN], inplace=True)
    
    df[CSV_LABEL_COLUMN] = df[CSV_LABEL_COLUMN].astype(int)

    df = df[df[CSV_LABEL_COLUMN].isin(VALID_LABELS)].copy()
    
    print("Traduzindo labels para o formato descritivo...")
    df[CSV_LABEL_COLUMN] = df[CSV_LABEL_COLUMN].map(LABEL_MAP)

    clean_df = df[[FINAL_TEXT_COLUMN, CSV_LABEL_COLUMN]].copy()
...
def save_json_pool(dataframe, file_path):
    ...
    data_list = dataframe.rename(columns={CSV_LABEL_COLUMN: 'label'}).to_dict('records')
...
def main():
    ...
    full_df = load_data_from_csv(CSV_FILE_PATH)
    if full_df is None:
        return
